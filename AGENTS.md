...
`setup.js` 把 `.yodo/src` **链接**到 `skills/yodo/src/`（Windows 用 junction；建不了链接的环境降级为整目录拷贝），数据目录 `.yodo/{task,tmp,record,session}` 独立，更新不碰。

日常开发：

```bash
node skills/yodo/setup.js                 # .yodo/src → skills/yodo/src
```
...
`.yodo/session/`：`sock`、`pid`、`log.jsonl`。run 没有 abort（关掉 client 进程即中止，holder 收到 socket close 后清理）。
...
- `stop` / `abort` / 5 分钟到 / 断线会关 discover、detach 自己挂过的 page、关录制窗。漏掉的第一发 document 可补 `late` 的当前 `outerHTML`；`late` 不装成 HTTP 200。
- 响应体 > 1KB 拆 `*.response.json` / `*.response.html`。
