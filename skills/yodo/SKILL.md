...
先跑脚本。`task/` 里有对得上的就直接跑并报告；没有一份直接对应、但现有脚本能拼，就在 `tmp/` 组合，试通了 `mv` 进 `task/` 再跑并报告。跑脚本做不成、又拼不出来，才录抓包：`record-start` 等人在新窗口做一遍，`record-stop` 交出 `recordDir`。有了抓包再写重放：对着 `timeline.jsonl` 和 request 在 `tmp/` 用 `yodo.run` + `pageForOrigin` + `evaluate` 写脚本，试通了留下再跑并报告。需要浏览器的命令会自己连；连不上就把 stdout 里的 `guide` 原样念给用户，停，等「好了」再重跑——写在下面每条会连浏览器的 use case 里。

# 1. 跑脚本

`task/*.js` 是已经跑通的自执行脚本。能直接跑就跑；能拼就在 `tmp/` 写新文件，不改原文件。

## 1.1 跑已有 task

按用户任务找 `task/*.js`（怎么查自定）。文件头有 `@summary`；用到的参数写在头注释里，对应 `argv` 位置。

能直接跑：`node <home>/.yodo/task/<name>.js [参数]`。参数走 `process.argv`，可缺省。不要改这份脚本。需要浏览器时命令会自己连。stdout 有 `need-install` / `need-chrome` / `need-remote-debugging` / `need-allow`：把 `guide` 原样念给用户，停，等用户回「好了」，再重跑刚才那条命令。不轮询，不改写 `guide`。只有 `error`、没有 `status`：命令没跑起来，停，对人说明没跑起来。
...
完成：已跑过对得上的文件，并对人讲了结果；或已把 `guide` 念出并在等「好了」；或现成 task `failure` 后已停并说明。没有对得上的、又拼不出来：去录抓包。没抓包不要先写脚本探接口。

## 1.2 组合现有 task

现有脚本能拼成这次要的：在 `tmp/` 写新文件，不改 `task/` 原文件。`import` `../task/_common/yodo.js`（`tmp` 里的文件用这条相对路径）。然后 `node <home>/.yodo/tmp/<name>.js [参数]`。

需要浏览器时命令会自己连。stdout 有 `need-install` / `need-chrome` / `need-remote-debugging` / `need-allow`：把 `guide` 原样念给用户，停，等用户回「好了」，再重跑刚才那条命令。不轮询，不改写 `guide`。只有 `error`、没有 `status`：命令没跑起来，停，对人说明；这次不算进 5 次。

同一接口最多 5 次有 `status` 的尝试。`failure` 不到 5 次就改 `tmp/` 再跑；满 5 次就停，写清原因，并对人说明这次做不成。没出现过 `success` 不准把脚本放进 `task/`。
...
完成：`mv` 完成并跑成功，已对人讲了结果；或满 5 次 `failure` 后已停并说明。拼不出来：去录抓包。

# 2. 录抓包

不知道站点真实请求时才录。只在新窗口做，已有窗不录。stop / abort 只关录制窗。5 分钟到会按 stop 归档。
...
完成：`stopped` 且手上有 `recordDir`，可以去写重放；或已说明没有抓到并停。

# 3. 写重放

只重放抓到的请求；没抓到的不补。只发网络请求，不操作 DOM，也不拿 DOM 快照来操作。正路是复用已登录的 origin 页，在页内 `fetch` / XHR。
...
同一接口最多 5 次有 `status` 的尝试。`failure` 且 403，多半是缺客户端签名，改走页内 XHR（站点常 monkey-patch `XMLHttpRequest`，会自动注入 `a_bogus` / `X-Bogus`），比手写逆向稳。页内太慢就减小数据量。`failure` 不到 5 次就改 `tmp/` 再跑。
...
5 次 `failure`：停，写清接口、原因、试了什么，并对人说明这次做不成。没出现过 `success` 不准把脚本放进 `task/`。卡住了杀 `session/pid`。stdout 不够再看 `session/log.jsonl`。
