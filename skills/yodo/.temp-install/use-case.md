...
## 跑 setup.js
目标：`~/.yodo/src` 是这次 skill 里的 `src/` 拷贝，依赖已装，数据目录已建或已补齐。
操作：
- Chrome 和 Node 都齐了才跑
...
- `setup.js` 一次做完：
  ...
  3. 把本 skill 的 `src/` 拷到 `~/.yodo/src`（先清空 dest，不拷 source 里的 `node_modules`）
  4. 在 dest `npm install`
  5. 调 `src/bin/init.js`：建 `task/` `temp/` `record/` `session/`，同步 `task/_common/`
- 分类：首次 → 原来没有 `.yodo/src`，跑完后有；再跑 → 覆盖 `.yodo/src`（更新或修缺目录），同一条命令
- 分类：覆盖 `.yodo/src`；不碰用户和 agent 写在 `task/` `temp/` 的脚本；不清 `record/` 里已有抓包；不要在 `.yodo/src` 写业务脚本
...
可观察结果：`~/.yodo/src` 存在，且其中有 `node_modules/tldts/dist/cjs/index.js`。
...
- 只缺数据目录时可以单独 `node ~/.yodo/src/bin/init.js`（建四个数据目录并同步 `_common/`；`setup.js` 已经会调）

| 目录 | 安装要留下 |
|---|---|
| `.yodo/src` | 源码拷贝 + 本地 `npm install` 的依赖。没有它 = 未初始化 |
| `.yodo/task` | 以后放已验证脚本。安装时带上 `_common/` |
| `.yodo/temp` | 尚未成功的候选 task |
| `.yodo/record` | 以后放抓包 |
