...
import { DomRecordStore } from "./dom.ts";
import type { RawCdpConnection } from "../browser/index.ts";
import { isRawRequest } from "./types.ts";
...

{
  ...
  const domStore = new DomRecordStore();
  ...
  const clicks = domStore.events.filter((e) => e.type === "click");
