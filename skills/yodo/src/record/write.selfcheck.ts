...
import { processTimelinePipeline } from "./pipeline.ts";
import { formatRecordStartStdout, writeArtifacts } from "./write.ts";
import type { RawEvent, RawRequest } from "./types.ts";
...
const domEvents = [{
  ...
  role: "button",
  name: "Go",
  target: { tag: "button", id: "go", classes: ["primary"] },
}];
const a = await writeArtifacts(dir, processed, domEvents, "demo");
...
assert.equal(lines[2].eventId, "dom_001");
assert.equal(lines[2].name, "Go");
assert.equal(lines[2].target, undefined);
assert.equal(lines[2].value, undefined);
